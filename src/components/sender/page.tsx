...
function Sender({}: Props) {
  ...
  useEffect(() => {
    ...
    // if (userdatas?.card?.number == reciever?.card?.number) {
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      {/* {!userdatas?.card?.number == reciever?.card?.number && ( */}
          ...
          <button
            ...
            onClick={(e) => {
              ...
              if (JSON.stringify(number).length === 0) {
                ...
                dispatch(patchUser(trData));
                dispatch(patchUser(senderData));
                // dispatch(setTransaction(transaction));
                console.log(trData);
                setproceeding(true);
                setTimeout(() => {
                  router.push(
                    `/success?transaction=${JSON.stringify(transaction)}`
                  );
                }, 2000);
              }
            }}
